...
def _cmd_fix(args: argparse.Namespace) -> int:
    ...
    if args.task:
        ...
        print(
            "Tell me what you're trying to do, and I'll figure out what's needed.",
            file=sys.stderr,
        )
        print(
            '  Example: agent-ready fix --task "deploy my portfolio"',
            file=sys.stderr,
        )
        return 1
...
def _cmd_verify(args: argparse.Namespace) -> int:
    ...
    if not args.capability:
        print("Which tool would you like me to check?", file=sys.stderr)
        print("  Example: agent-ready verify vercel_cli", file=sys.stderr)
        return 1

    cap = by_id(args.capability)
    if not cap:
        print(f"I don't know about '{args.capability}'. Try one of these:", file=sys.stderr)
        for c in by_id.__self__.values() if hasattr(by_id, "__self__") else []:
...
def _cmd_undo(args: argparse.Namespace) -> int:
    ...
    if not args.capability:
        print("Which tool would you like me to remove?", file=sys.stderr)
        print("  Example: agent-ready undo vercel_cli", file=sys.stderr)
        return 1

    cap = by_id(args.capability)
    if not cap:
        print(f"I don't know about '{args.capability}'.", file=sys.stderr)
        return 1
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    detect_p = sub.add_parser(
        "detect",
        help="See what's missing for a task",
    )
    detect_p.add_argument(
        "--from", dest="source", help="Path to trace / scorecard / log (default: stdin)"
    )
    detect_p.add_argument("--task", help="What you're trying to do, e.g. 'deploy my site'")
    detect_p.add_argument("--json", action="store_true", help="Machine-readable output")
    detect_p.set_defaults(func=_cmd_detect)

    fix_p = sub.add_parser(
        "fix",
        help="Set up everything that's missing",
    )
    fix_p.add_argument(
        "--from", dest="source", help="Path to trace / scorecard / log (default: stdin)"
    )
    fix_p.add_argument("--task", help="What you're trying to do, e.g. 'deploy my site'")
    fix_p.add_argument(
        "--dry-run", action="store_true", help="Show what would happen without running anything"
    )
    fix_p.add_argument(
        "--non-interactive", action="store_true", help="Skip approval prompts (for scripting)"
    )
    fix_p.set_defaults(func=_cmd_fix)

    verify_p = sub.add_parser("verify", help="Check if a tool is set up correctly")
    verify_p.add_argument("capability", nargs="?", help="Tool ID, e.g. vercel_cli")
    verify_p.set_defaults(func=_cmd_verify)

    undo_p = sub.add_parser("undo", help="Remove what was set up")
